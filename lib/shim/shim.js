...
function bindSegment(nodule, property, segment, full) {
  ...
  if (segment && segment.transaction && !segment.transaction.isActive()) {
    this.logger.trace('Transaction for segment is not active (%s)', segment.name)
    return nodule
  }
  ...
  var wrapped = this.wrap(nodule, property, function wrapFunc(shim, func) {
    ...
    segment = segment || shim.getActiveSegment()
    if (!segment) {
      return func
    }
...
function storeSegment(obj, segment) {
  const currentSegment = segment || this.tracer.getSegment()
  if (
    currentSegment &&
    currentSegment.transaction &&
    !currentSegment.transaction.isActive()
  ) {
    return
  }

  this.setInternalProperty(obj, '__NR_segment', currentSegment)
...
function applySegment(func, segment, full, context, args, inContextCB) {
  ...
  if (segment.transaction && !segment.transaction.isActive()) {
    this.logger.trace('Transaction is not active, not applying segment: %s', segment.name)
    return fnApply.call(func, context, args)
  }
