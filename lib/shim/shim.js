...
function Shim(agent, moduleName, resolvedName) {
  ...
  // TODO: THIS IS BAD, INJECT THIS
  this._contextManager = agent._contextManager
  this.logger.warnOnce('THIS IS BAD, INJECT THIS')
...
function setActiveSegment(segment) {
  // TODO: seems like shouldn't do this anymore?
  // Should be wrapped or use context manager? Potentially w/ a stack?
  // return (this.tracer.segment = segment)

  // TODO: this is allocate-y
  const context = { segment }
  this._contextManager.setContext(context)
...
function storeSegment(obj, segment) {
  const segmentToStore = segment || this._contextManager.getContext().segment
  this.setInternalProperty(obj, '__NR_segment', segmentToStore)
}
...
function applySegment(func, segment, full, context, args, inContextCB) {
  ...
  try {
    ...
    return this._contextManager.runWithContext(segmentContext, func, context, args)
