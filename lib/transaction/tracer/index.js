...
function bindFunction(handler, segment, full) {
  ...
  const currentSegment = segment || this.segment
  if (
    currentSegment &&
    currentSegment.transaction &&
    !currentSegment.transaction.isActive()
  ) {
    logger.trace('no active transaction for seg %s', currentSegment.name)
    return handler
  }

  // return _makeWrapped(this, handler, segment || this.segment, !!full)
  return _makeWrapped(this, handler, currentSegment, !!full)
}
