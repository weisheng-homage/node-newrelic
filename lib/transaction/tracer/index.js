...
// // TODO: actually transition to a context manager first?
// Object.defineProperty(Tracer.prototype, 'segment', {
//   get: function segmentGetter() {
//     throw new Error('Bad! Dont use this!')
...
// TODO: prob ideally avoid using tracer for this in future?
function getTransaction() {
  //const { segment } = this._contextManager.getContext()
  const segment = this._contextManager.getContext()

  // TODO: if context held segment and transaction could just grab transaction?
  const isActive = segment && segment.transaction && segment.transaction.isActive()
  if (isActive) {
...
function getSegment() {
  // TODO: prob eventually avoid grabbing from here?
  // const { segment } = this._contextManager.getContext()
  const segment = this._contextManager.getContext()
  return segment
}
...
function getSpanContext() {
  ...
  return segment && segment.getSpanContext()
}
...
function transactionProxy(handler) {
  ...
  const wrapped = function wrapTransactionInvocation() {
    ...
    // const { segment } = tracer._contextManager.getContext()
    const segment = tracer._contextManager.getContext()
    if (segment) {
...
function transactionNestProxy(type, handler) {
  ...
  const wrapped = function wrapTransactionInvocation() {
    ...
    // let { segment } = tracer._contextManager.getContext()
    let segment = tracer._contextManager.getContext()
...
function bindFunction(handler, segment, full) {
  ...
  // TODO: only pull if needed
  // const { segment: activeSegment } = this._contextManager.getContext()
  const activeSegment = this._contextManager.getContext()
  return _makeWrapped(this, handler, segment || activeSegment, !!full)
}
function _makeWrapped(tracer, handler, active, full) {
  ...
  function wrapped() {
    // TODO: this is very allocate-y... may cause problems
    // coudl just track segments directly for intial implemnentation
    // Could have each segment have a context object that gets propogated and only
    // turned into new object upon extra metadata
    // Other?
    ...
    try {
      //const segmentContext = { segment: active }
      const segmentContext = active
      // Don't love passing in "this" but otherwise need to bind up-front?
      // TODO: based on how context manager currently setup... arguments spreads weird.

      // return tracer._contextManager.runWithContext(segmentContext, handler, this, arguments)

      return tracer._contextManager.runWithContext(segmentContext, () => {
        // const ctext = tracer._contextManager.getContext()
        return handler.apply(this, arguments)
      })
    } finally {
      // TODO: any way to decouple this?
      if (active && full) {
        active.touch()
      }
    }
