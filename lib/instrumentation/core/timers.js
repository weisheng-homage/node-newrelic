...
function initialize(agent, timers, moduleName, shim) {
  // TODO: cleaner/better way to do this
  if (!agent.config.feature_flag.new_async_context) {
    console.log('new_async_context: true skipping process timers instrumentation')
  ...
  function instrumentTimerMethods(nodule) {
    ...
    // TODO: way to measure w/o context propagation?
    shim.record(nodule, asynchronizers, recordAsynchronizers)

    if (!agent.config.feature_flag.new_async_context) {
      console.log('new_async_context: true skipping setImmediate instrumentation')
...
function makeWrappedPromisifyCompatible(shim, timers) {
  ...
  // Don't touch setImmediate as it was not instrumented
  if (shim.agent.config.feature_flag.new_async_context) {
    return
  }
