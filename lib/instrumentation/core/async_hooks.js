...
function initialize(agent, shim) {
  // TODO: Ideally, managing async hooks would be part of context manager but here it is
  // determining whether or not to use old state tracking so not bothering in prototype.
  if (agent.config.feature_flag.new_async_context) {
    logger.info(
      'New async context enabled... not enabling manual async_hooks or promise instrumentation'
    )
    return
  }
