...
module.exports = function initialize(agent, redis, moduleName, shim) {
  if (redis && redis.createClient) {
    ...
    shim.wrapReturn(redis, 'createClient', function wrapCreateClient(shim, fn, name, client) {
      // debugger
      shim.recordOperation(client, 'sendCommand', function wrapSendCommand(shim, _, __, args) {
        debugger
      })
    })
  } else {
