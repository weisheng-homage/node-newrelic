...
function instrument(agent, nodule, moduleName, resolvedName) {
  const moduleInstrumentConfig = agent.config.instrumentation[moduleName]
  if (moduleInstrumentConfig && moduleInstrumentConfig.enabled === false) {
    logger.info(
      '**** ' +
      'Skipping %s. ' +
      'It has been set to enabled: false in the instrumentation configuration',
      moduleName
    )
    return nodule
  }
